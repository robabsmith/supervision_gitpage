...
###########################################################################
    ...
    write.xlsx(dat_clean, "/R output data/dat_c.xlsx")
    
    # Export data to an Excel Spreadsheet___MAC
    write.xlsx(dat, "/R output data/dat.xlsx")
    
    # Export data to an Excel Spreadsheet ___Windows
    write.xlsx(dat_clean, "c:/R output data/dat_c.xlsx")
    
    # Export data to an Excel Spreadsheet ___Windows
    write.xlsx(dat, "c:/R output data/dat.xlsx")
    
###########################################################################
###########################################################################
    # Export dat to CSV file
    write.csv2(dat,"/R output data/dat.csv")
    
    # Export dat_clean to CSV file
    write.csv2(dat_clean,"/R output data/dat_c.csv")
###########################################################################
###########################################################################
...
###########################################################################
# Households savings and B9
###########################################################################
dat$h_savings_pos <-  dat$w_h_R + dat$b2_h + dat$pi_h_R + dat$str_ob_h + dat$str_oc_h_R + dat$cpen_h
dat$h_savings_neg <- (-dat$c_h) + (-dat$tc_h) + (-dat$str_c_h) + dat$pi_h_P + dat$str_oc_h_P
h_savings_ts <- ts((dat$h_savings_pos - dat$h_savings_neg), start = 1995, end = 2016, freq = 1)

h_savings_ts

dat$h_B9_pos <-  dat$w_h_R + dat$b2_h + dat$pi_h_R + dat$str_ob_h + dat$str_oc_h_R + dat$cpen_h + dat$captr_h_R
dat$h_B9_neg <- (-dat$c_h) +(-dat$i_h) + (-dat$tc_h) + (-dat$str_c_h) + dat$pi_h_P + dat$str_oc_h_P + dat$captr_h_P
h_B9_ts <- ts((dat$h_B9_pos - dat$h_B9_neg + dat$np_h), start = 1995, end = 2016, freq = 1)
...
dat$g_savings_pos <-  
dat$g_savings_neg <- 
g_savings_ts <- ts((dat$g_savings_pos - dat$g_savings_neg), start = 1995, end = 2016, freq = 1)

g_savings_ts

dat$g_B9_pos <-  
dat$g_B9_neg <- 
g_B9_ts <- ts((dat$g_B9_pos - dat$g_B9_neg + dat$np_h), start = 1995, end = 2016, freq = 1)
...
dat$nf_savings_pos <-  
dat$nf_savings_neg <- 
nf_savings_ts <- ts((dat$nf_savings_pos - dat$nf_savings_neg), start = 1995, end = 2016, freq = 1)

nf_savings_ts

dat$nf_B9_pos <-  
dat$nf_B9_neg <- 
nf_B9_ts <- ts((dat$nf_B9_pos - dat$nf_B9_neg + dat$np_nf), start = 1995, end = 2016, freq = 1)
...
dat$f_savings_pos <-  
dat$f_savings_neg <- 
f_savings_ts <- ts((dat$f_savings_pos - dat$f_savings_neg), start = 1995, end = 2016, freq = 1)

f_savings_ts

dat$f_B9_pos <-  
dat$f_B9_neg <- 
f_B9_ts <- ts((dat$f_B9_pos - dat$f_B9_neg + dat$np_f), start = 1995, end = 2016, freq = 1)
